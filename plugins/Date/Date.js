(function() {
	...
	JSYG.Date.prototype = {
		...
		add : function(units,nb) {
			...
			switch ( units.toLowerCase().replace(/s$/i,'') ) {
				...
				case "second" :
					...
					val.mod != 0 && this.add("milliseconds", Math.round(val.mod * 1000) );
					break;
		 ...
		 * @returns {JSYG.Date}
		 * @example
		 * new JSYG.Date("2012-12-15T23:25:00").reset("day").toString("DD/MM HH:MI:SS"); //15/12 00:00:00
		 * new JSYG.Date("2012-12-15T23:25:00").reset("month").toString("DD/MM HH:MI:SS"); //01/12 00:00:00
		 */
		reset : function(unit) {
		...
		clone : function() { return new JSYG.Date(+this.date); }
	};
